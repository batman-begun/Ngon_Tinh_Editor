...
## Dictionary hậu kiểm mặc định

App có sẵn một dictionary hậu kiểm được code thẳng trong `omni_novel_editor/dictionary/rules.py` và bản CSV tham khảo ở `omni_novel_editor/dictionary/dictionary_sample.csv`.

Dictionary này là layer xử lý thô bằng code/regex trước và sau Gemini:

1. Trước Gemini: app thay các từ/cụm/dấu câu đã biết để giảm token và giảm khả năng model tự sáng tạo.
2. Trong prompt Gemini: app chỉ đưa các rule liên quan đến chunk hiện tại vào phần `DICTIONARY STRICT RULES`.
3. Sau Gemini: app chạy lại dictionary và cleaner TTS để ép rule strict, ví dụ loại dấu nháy/dấu hội thoại mà model tự thêm lại.
4. Các câu chửi tục trực diện như `đụ má`, `địt mẹ`, `đĩ mẹ` được normalize thành `đờ mờ`; các câu cảm thán suồng sã như `vãi thật` không bị đổi máy móc thành `đờ mờ`.

Một số input có nhiều biến thể bằng dấu `/`, ví dụ `tự sát/ tự tử/ tự vẫn`, sẽ được tách thành nhiều rule cùng output. Nếu một input có nhiều output khác nhau, ví dụ `ship`, app không replace cứng mà đưa vào prompt để Gemini chọn theo ngữ cảnh.
